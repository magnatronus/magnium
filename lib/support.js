 ...
 *  - why is recursion so painful and yet satifying ?
 */
buildUI = (element, parent = false, level=0) => {
    ...
    const component = `${createComponent(element.name, element.attr)}`;
    ...
    processFile: (file) =>{
        if(fs.lstatSync(file).isFile()){
            ...
            while (match = exp.exec(text)) {
                ...
                const ui = generateView(text.substring(match.index  + 7, exp.lastIndex - 1));
